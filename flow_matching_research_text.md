...
この教材は、Flow Matchingを「直感」「数式」「PyTorchコード」の対応で理解し、実際にモデルを組めるようになるための日本語教材である。AI、ニューラルネットワーク、PyTorchの基礎は前提とするが、生成モデルやDiffusionモデルを使った経験は前提としない。

第0部で生成モデル未経験者向けの準備を行い、第1部ではFlow Matchingの直感、Conditional Flow Matching（CFM）の理論、最小コード、2D実験を一続きで学ぶ。その後、Diffusionとの対応、source点とtarget点のペアリングを変えるOptimal Transport CFM（OT-CFM）、軌道の直線化を扱うRectified Flow、サンプリング、実モデル、評価設計へ進む。目標は、読者が次の4点を自分の言葉とコードで説明できるようになることである。
...
## 0.4 Flow Matchingの全体像を先に言葉でつかむ
...
ニューラルネットワークには、途中地点と現在時刻を見せ、その場所の正解矢印を予測させる。同じような場所へ異なる組から矢印が集まる場合、モデルはそれらをまとめた進行方向を学ぶ。こうして、空間のどこにいても次の進行方向を答える「矢印の地図」ができる。
...
「矢印を空間中にびっしり描いた地図」と考えるとよい。ただし実装では全地点の矢印を保存するわけではない。ニューラルネットワークが、問い合わせられた場所に対してその都度ベクトルを計算する。

### 0.5.3 Flow Matchingでは時刻によって地図も変わる
...
同じ場所 $x$ でも、生成の序盤と終盤では進むべき方向が違うことがある。そのため $v_\theta(x)$ ではなく、時刻依存ベクトル場 $v_\theta(t,x)$ を使う。下付きの $\theta$ は、ニューラルネットワークの学習可能なパラメータをまとめて表す。
...
左辺は位置が時間に対してどれくらい変わるか、右辺はモデルが現在の時刻と場所で返した速度である。Flow Matchingではニューラルネットワークが右辺を学び、生成時にこの規則を少しずつたどる。
...
| $v_\theta(t,x_t)$ | ニューラルネットワークが予測する速度 | model output |

最初からすべての用語を暗記する必要はない。第1部の前半では、上の8記号がコードのどのtensorに対応するかだけを追えばよい。

## 0.8 初読の進み方

生成モデルが初めてなら、次の順で読む。各部末の「公式実装ではどこを見るか」は、最初の一周では飛ばしてよい。

1. **第0部**: 乱数をデータへ変形する、という生成モデルの全体像をつかむ。
2. **第1部**: CFMの直感、数式、loss、最小コード、2D実験を一続きで理解する。
3. **第2部**: 自分でmodel、loss、学習、生成を接続する。
4. **第3部と第4部**: Diffusionを初めて学び、noise predictionとvelocity predictionを比較する。
5. **第5部と第6部**: couplingとRectified Flowを研究上の設計変数として学ぶ。
6. **第7部**: 速度場モデルをMLPからU-NetやDiTへ広げる。
7. **第8部**: NLL、MMD、FID、KIDなどの評価とablationを学ぶ。
8. **第9部**: 評価指標を使い、solverとNFEの比較実験を行う。
9. **第10部**: 全部品の接続を一枚の設計図で復習する。
10. **第11部（必要になってから読む）**: 公開済みモデルのfine-tuning、重みの再利用、互換性、ライセンスを扱う。最小モデルを自作するだけなら初読では飛ばしてよい。

数式で止まったときは、その式を完全に証明してから進む必要はない。まず「どのtensorを作る式か」「学習時と生成時のどちらで使うか」を確認し、コードを動かしてから理論節へ戻る。

## 0.9 用語集

この節は辞書として使うことを目的としている。初読で全部読む必要はなく、本文を読んだ後に戻ってきてもよい。
...
**Flow Matching**:
source分布からtarget分布へ至る確率経路を先に設計し、その経路を運ぶ速度をニューラルネットワークで回帰する学習方法。学習後は、得られた速度場をODEとして積分して新しいサンプルを生成する。

**Conditional Flow MatchingまたはCFM**:
端点ペアなどの条件を固定すれば計算できる条件付き速度を教師信号にするFlow Matching。モデルには通常その条件を直接渡さず、時刻と途中状態から速度を予測させることで、生成時に使える周辺速度場を学ぶ。
...
**速度場**:
時刻 $t$ と場所 $x$ を受け取り、その場所の点が次に進むべき方向を返す関数。この教材ではニューラルネットワークで近似する速度場を $v_\theta(t, x)$ と書く。
...
**solverまたは数値積分法**:
samplerの内部で、ODEに沿った1 stepの進み方を計算する方法。Euler、Heun、RK4などがあり、同じtime gridでも1 step当たりのモデル呼び出し回数と数値誤差が異なる。

**time gridとstep**:
ODEを数値的に解くために、連続時間区間を並べた離散的な時刻列をtime gridと呼ぶ。隣り合う時刻の間を1回更新することが1 stepである。step数とNFEはsolverによって一致しない。

**NFE**:
Number of Function Evaluationsの略。生成時にニューラルネットワーク `v_theta` を何回呼ぶかを表す。Euler samplerで64ステップ進めるなら、だいたいNFEは64である。NFEが小さいほど速いが、軌道が曲がっていると誤差が出やすい。

**NLL**:
Negative Log-Likelihood（負の対数尤度）の略。**尤度**とは、「学習済みモデルから見て、実際のデータがどれくらい起こりやすいか」を表す量である。NLLはその値へ負号を付けて対数を取った指標で、モデルがtargetデータへどれだけ高い確率密度を割り当てるかを測る。低いほどよいが、画像では離散pixelのdequantization、値域、次元数、密度計算に使うsolverをそろえなければ比較できない。
...
**KID**:
Kernel Inception Distanceの略。実画像と生成画像のInception特徴にkernelを適用し、分布差をMMD²として測る画像生成指標。FIDと異なり特徴分布をGaussianと仮定せず、不偏推定量を構成できるが、有限サンプルでは推定値が揺れ、負になることもある。

**lossまたは損失関数**:
モデルの予測が正解からどれくらい外れているかを測る関数。Flow Matchingでは、予測速度 $v_\theta(t, x_t)$ と正解速度 $u_t$ の二乗誤差を使うのが基本である。

**予測ターゲット**:
ニューラルネットワークに何を出力させるかという意味。Flow Matchingでは速度を予測させるが、Diffusion系ではノイズ、データ、velocityなどを予測させる場合がある。
...
**時刻埋め込み**:
スカラーの時刻 $t$ を、ニューラルネットワークが扱いやすいベクトルに変換したもの。$t$ をそのまま1個の数として渡すより、sin/cosなどで複数次元に広げると、時間ごとの違いを学びやすい。
...
**coupling**:
どのsourceサンプル $x_0$ とどのtargetサンプル $x_1$ をペアにするかの決め方。第1部の最小例ではランダムにペアにするが、後のOT-CFMでは、このペアリングを工夫して軌道をより素直にする。

**Optimal Transport CFMまたはOT-CFM**:
sourceとtargetを独立にペアにする代わりに、移動距離などのcostが小さくなるOptimal Transport couplingを使うCFM。教材コードのgreedy couplingは差し替え位置を学ぶ近似例であり、厳密なOT solverではない。

**Rectified Flow**:
端点ペアを直線補間し、その直線速度を学ぶflowモデル。Flow Matchingと共通する速度回帰の形を持つが、Reflowによって学習用ペアを作り直し、生成軌道をより直線に近づける考え方を明示的に扱う。

**Reflow**:
学習済みflowでsourceサンプルを生成側へ運び、その出発点と終点を新しいペアとして次のRectified Flowを学習する手続き。モデルを小さくする操作ではなく、学習に使うcouplingを更新する操作である。

**モデル容量**:
ニューラルネットワークがどれくらい複雑な関数を表せるかを表す言い方。層が浅すぎる、隠れ次元が小さすぎる、時間情報をうまく使えない、といった場合はモデル容量が足りず、正しい速度場を表せないことがある。

**checkpoint**:
学習途中または学習後のモデル重みと、モデル構成や実験条件を保存したファイル。再生成・評価・追加学習では、重みだけでなくpath、coupling、モデル構成などが学習時と一致することを確認する。

**seedまたは乱数seed**:
擬似乱数の初期状態を指定する値。同じseedは実験の再現を助けるが、単一seedだけでは結果のばらつきを評価できないため、研究結果では複数seedも比較する。

# 第1部 Flow Matchingの基礎・理論・最小実装

## 1. 分布を運ぶ生成モデルとしてのFlow Matching

> **この章で扱う概念**: Flow Matchingは、source分布からtarget分布へサンプルを運ぶ時間依存の速度場を、ニューラルネットワークで学習する生成モデルである。完成データを直接出力するのではなく、各時刻・各位置で「次にどちらへ進むか」を表すベクトルを予測し、その速度場をODEとして積分して生成する。
...
この「次に進むべき方向」を返す関数が速度場である。ニューラルネットワークは
...
- coupling、time sampler、path、教師速度、loss、optimizerは、矢印の地図を作る**学習側**の部品である。
- velocity modelは、学習時に矢印を練習し、生成時に矢印を答える**共有部品**である。
- time grid、ODE solver、sampler、NFEは、学習済みの地図をたどる**生成側**の部品である。

この先の章では、これらを一つずつ数式とコードへ落としていく。今は用語を暗記する必要はない。「ペアを作る → 練習経路と正解矢印を作る → 矢印の地図を学ぶ → 新しいノイズを地図に沿って歩かせる」という関係だけをつかめばよい。第10部のまとめで再び整理する。
...
ここで、点の軌道 $x_t$ と分布 $p_t$ を分けて考える。図1に示すように、ODEで直接動くのは1つ1つの点である。一方、分布 $p_t$ は、大量の点を同じ速度場で動かしたときの集まりとして変形する。

![点の軌道と分布の変形](figures/particle_distribution_flow.svg)

*図1　1個の点が描く軌道と、多数の点が作る分布の変形*

図1に示した区別は、Flow Matchingでかなり大切である。$x_t$ は1個のサンプルの位置、$p_t$ は時刻 $t$ におけるサンプル全体の分布である。学習ではミニバッチの点を使うが、理論で言う「分布が動く」とは、すべての点を同じ速度場で動かしたときに密度の形が変わることを指す。
...
この $u_t$ を教師信号として、ニューラルネットワークの出力 $v_\theta(t, x_t)$ を近づける。
...
[losses.py](src/fm_minimal/losses.py) では、Linear pathに関する部分を次の2行として読める。
...
3. たくさんのペアでこれを繰り返す。
4. ニューラルネットワークは、同じような場所・時刻に来る多くの教師速度を平均するように学ぶ。

これがConditional Flow Matchingの基本的な気持ちである。

### 4.2 条件付き経路

ここでいう「条件」は、クラスラベルやテキストを生成モデルへ入力する**条件付き生成の条件**ではない。学習用に選んだsource点とtarget点のペア
...
多数の条件付き経路を混ぜ、ペア $z$ を平均して見えなくする操作を**周辺化**という。周辺化した後に残る時刻 $t$ の分布 $p_t$ が周辺分布であり、時刻ごとに並べた $p_t$ 全体が確率経路（probability path）である。
...
1. source点とtarget点を1組選ぶ。
2. その間の時刻を1つ選び、途中地点 $x_t$ を作る。
3. そのペアを結ぶ正解の矢印 $u_t$ を作る。
4. ニューラルネットワークに $(t,x_t)$ だけを見せて矢印を予測させる。
5. 予測矢印と正解矢印のずれを二乗し、ミニバッチで平均する。
...
| コード | 数式上の役割 |
|---|---|
| `x_t` | ニューラルネットワークへ入力する途中地点 |
| `u_t` | ペアから計算できる教師速度 |
| `pred` | $v_\theta(t,x_t)$、予測した速度 |
| `flatten(1).sum(dim=1)` | 各サンプルについて全成分の二乗誤差を足す |
| `mean()` | ミニバッチ平均で期待値を近似する |
...
5. 学習後、samplerが新しい標準正規ノイズを用意し、内部のEuler solverで動かす。

実行例を示す。以降のコマンドは、カレントディレクトリの直下に `src`、`_outputs`、`figures` がある状態で実行する。リポジトリを取得した場合は、本教材ファイルと `src` が見えるディレクトリへ移動してから実行する。Pythonの起動コマンドは環境によって `python`、`python3`、`py` のいずれかになるため、本教材では `python` と表記する。例はPowerShell形式である。Bashやzshでは、パス区切りの `\` を `/` に、行末のバッククォートを `\` に置き換えるか、コマンドを1行で入力する。
...
Euler、Heun、RK4は、連続時間ODEの解を有限回の速度評価で近似する**明示的数値積分法**である。Eulerは1 stepの始点だけ、Heunは始点と仮の終点、RK4は区間内の4地点で速度を調べる。高次のsolverほど1 stepを丁寧に計算するが、モデルを呼ぶ回数も増える。そのため、solverはstep数だけでなく、モデル評価回数であるNFEと離散化誤差を合わせて比較する。
...
### 7.3.1 公式ライブラリで見かけるCFM variants
...
|---|---|---|---|
| `ConditionalFlowMatcher` | $z=(x_0,x_1)$, independent coupling | sourceとtargetを独立にペアにする基本CFM | 第1部、第2部 |
| `TargetConditionalFlowMatcher` | $z=x_1$ | target点ごとの条件付きGaussian pathを使う | 第3章 |
| `ExactOptimalTransportConditionalFlowMatcher` | $z=(x_0,x_1)$, OT coupling | ペアリングをOT計画に変える | 第5部 |
| `SchrodingerBridgeConditionalFlowMatcher` | entropic OT coupling | 確率的bridgeやscoreとの接続が入る | 発展補足 |
| `VariancePreservingConditionalFlowMatcher` | independent coupling + VP/Gaussian path | pathを分散保存的なGaussian pathに変える | 第3章 |
...
### 7.4 CFMの実装部品をTorchCFMと対応づける

TorchCFMでは、CFMの種類がFlowMatcherクラスとして分かれている。第1部を読んだ直後は、次の順で見る。
...
は「その場所を、どれくらいの確率質量が、どちら向きに通過しているか」を表す流量である。流量の出入りと密度変化の帳尻を合わせる式が、**連続の式**である。
...
第1部から公式実装へ進むときは、モデルの大きさより先に、`path -> conditional velocity -> loss -> solver` の対応を追う。Meta `flow_matching` では2D exampleとpath・solver、TorchCFMではFlowMatcherが返す $t,x_t,u_t$ を確認する。具体的な対応表とURLは、6.5節、7.4節に示した。

# 第2部 自分でモデルを組んで学習する

## 12. 速度場モデルを自分で定義する

> **この章で扱う概念**: 速度場モデルは、時刻 $t$ と現在状態 $x_t$ を受け取り、同じデータ空間上の速度ベクトル $v_\theta(t,x_t)$ を返すニューラルネットワークである。画像そのものを一度に予測するモデルではなく、ODEの右辺を表現する関数である。
...
ただし、$t$ はただの1つの数である。ニューラルネットワークが時間の違いを使いやすくするために、$t$ を複数次元のベクトルへ変換する。この変換が時刻埋め込みである。
...
# 第3部 Diffusion実装の最小補講

## 17. Diffusion実装では何をしているか

> **この章で扱う概念**: Diffusionモデルは、データを段階的にノイズ化する前向き過程を定め、その中間状態から逆向きの生成に必要な情報をニューラルネットワークで予測する生成モデルである。Flow Matchingと似た時刻条件付きモデルを使うが、学習targetと生成更新式はschedulerの定義に依存する。
...
```powershell
python .\src\train_diffusion_toy.py --steps 2000 --batch 512 --device cpu
```
...
この横並びを理解したうえで第3章のGaussian Pathを振り返ると、同じ中間状態をDiffusionとFlow Matchingがどう読み替えるかが見えやすくなる。

## 19. 第3部のまとめと公式実装への入口

### 19.1 確認事項

- Diffusionの学習では、既知のnoiseを混ぜた中間状態から予測targetを回帰する。
- 同じ $x_t$ を使っても、noise predictionとFlow Matching velocityでは教師信号が異なる。
- 生成時はscheduler、時間向き、予測targetから更新式を読み解く。
...
# 第4部 Diffusion表記とFlow Matchingの対応

## 20. DiffusionをFlow Matchingの言葉で見る
...
まず、第4部全体を読むための包含関係を押さえる。

> **Flow Matchingは「source分布からdata分布まで、どのような確率経路を通るか」を設計し、その経路を動かす速度場を学ぶ一般的な方法である。経路としてGaussian noising pathを選ぶと、拡散モデルで使われる中間分布と対応する。したがって、確率経路の設計という観点では、拡散モデルはFlow Matchingの特殊な場合の一つとして捉えられる。**

直感的には、Flow Matchingが「出発地と目的地を結ぶ道路の作り方まで選べる枠組み」だとすれば、拡散モデルは「途中の状態をGaussian noiseとの混合で作る道路」を選んだ場合に対応する。

```text
Flow Matchingという一般的な設計枠組み
├─ Linear pathを選ぶ
├─ Gaussian pathを選ぶ  ← Diffusionと深く対応する
├─ OT couplingを組み合わせる
└─ その他の確率経路を設計する
```
...
同じGaussian pathを使うとき、両者は同じ道を別の向きから眺めていると考えられる。ただし、道が似ていても「モデルに何を答えさせるか」や「生成時にどう進むか」まで必ず同じになるわけではない。この違いは次節以降で整理する。

### 20.2 理論: 確率経路を選ぶとは何か

Flow Matchingでは、source分布 $p_0$ とdata分布 $p_1$ を結ぶ確率経路

$$
(p_t)_{t\in[0,1]}
$$

を考え、それを運ぶ速度場 $v_t(x)$ を学ぶ。ここで「経路を自由に選べる」とは、何を選んでも学習が同じになるという意味ではない。確率経路 $p_t$ を変えると、次のものがまとめて変わる。

- 学習用の中間点 $x_t$ がどこに現れるか。
- 正解として与える条件付き速度 $u_t$ がどの向きになるか。
- 学習後の生成軌道がどれくらい曲がるか。
- 少ないNFEで解きやすいか。
- 条件付き経路からサンプルと速度を簡単に計算できるか。

つまり、確率経路は単なる見た目の違いではなく、**学習データの作り方、教師信号、生成の難しさを同時に決める設計変数**である。実際のCFMでは、任意の周辺分布 $p_t$ を直接扱うのではなく、サンプル生成と速度計算が容易な条件付き経路を設計して学習する。

Gaussian pathを選んだ場合、中間点は典型的に

$$
x_t = \alpha(t)x_1 + \sigma(t)x_0
$$

と書ける。これは、データ成分とGaussian noise成分を時刻に応じて混ぜるDiffusionの中間状態と同じ形である。Diffusionで使うnoise scheduleは、Flow Matchingから見ると「Gaussian pathの係数 $\alpha(t),\sigma(t)$ をどう選ぶか」に対応する。

ただし、拡散モデルがFlow Matchingの特殊な場合だという説明は、主に**確率経路とその速度場の対応**について述べている。拡散モデルの逆向きSDE、確率的sampler、score predictionまでが、そのまま通常のODE型Flow Matchingと同一になるわけではない。同じ中間分布を共有できても、何を予測するか、ODEとSDEのどちらで生成するかは別に確認する必要がある。この違いは18章と19章で整理する。
...
### 20.4 Stochastic Interpolantsとして見る
...
## 21. 何を予測するか: noise, data, velocity
...
### 21.2 Gaussian pathからdata/noiseを解く
...
### 21.3 Flow Matching velocityとの関係
...
### 21.5 Diffusion実装からFM実装へ移るときの翻訳表
...
## 22. DiffusionとFlow Matchingは同じなのか

> **この章で扱う概念**: DiffusionとFlow Matchingは、時間依存の中間分布と時刻条件付きニューラルネットワークを共有できるが、生成過程と学習目的を定める枠組みは同じではない。どの経路を選び、何を予測し、ODEとSDEのどちらで生成するかを指定して初めて具体的な対応が決まる。
...
# 第5部 Optimal Transport CFMとcoupling設計

## 24. 独立couplingからOT couplingへ

> **この章で扱う概念**: 第1部で導入したcouplingを設計変数として扱い、ペア間の輸送コストを小さくするOT-CFMへ進む。

couplingの定義、独立couplingの式、`random_coupling` の役割については4.4節を参照してほしい。ここでは同じCFM lossを保ったまま、ペアリングを最適化すると何が変わるかを見る。

### 24.1 この部で比較するもの

| 比較対象 | ペアの決め方 | この部で調べること |
|---|---|---|
| 独立coupling | sourceとtargetを独立に選ぶ | 基準となる生成品質と軌道 |
| OT coupling | 輸送コストが小さい組を選ぶ | 教師速度、軌道、少NFE品質が改善するか |

両者でpath、モデル、optimizer、solverを固定すれば、観測した差をcouplingの違いとして解釈しやすい。

## 25. Optimal Transport CFMの考え方
...
# 第6部 Rectified FlowとReflow

## 28. Rectified FlowはFlow Matchingの何を変えるのか

> **この章で扱う概念**: 1-Rectified Flowの学習lossは、本教材のLinear Path CFMと同じ形である。Rectified Flowとして加わる中心的な考え方は、学習後の実軌道を調べ、そのflowが作るcouplingでペアを作り直し、より直線的で少ないNFEでも追いやすいflowへ再学習することである。

### 28.1 まず結論: lossを別物にするのではない

Flow Matchingは、確率経路と教師速度を決め、その速度をニューラルネットワークで回帰する広い枠組みである。Rectified Flowはその枠組みの外へ出るのではなく、直線補間を使った速度回帰を出発点にして、**学習後のflowをさらにまっすぐにする手順**を重視する。

本教材の最小Linear Path CFMと1-Rectified Flowを比べると、最初の学習では多くの部品が同じである。

| 観点 | Linear Path CFM | Rectified Flowで加わる視点 |
|---|---|---|
| 条件付きpath | 端点を直線で結ぶ | 同じ直線補間から始める |
| 教師速度 | $x_1-x_0$ | 同じ教師速度を使う |
| loss | 速度のMSE | 1回目は同じMSE |
| velocity model | $v_\theta(t,x)$ | 入出力は同じ |
| 生成 | ODE solverで速度場を積分する | 同じODE生成を使う |
| 学習後に見るもの | 生成品質 | 生成品質に加えて軌道の直線性と少NFE品質を見る |
| 次の学習ペア | 通常は元のcouplingを使う | 学習済みflowが作るcouplingへ更新する |

したがって、「Rectified FlowはCFM lossを新しい式へ交換した手法」と理解するとずれる。1回目の学習は同じでも、その後に軌道を診断し、ペアを作り直して再学習するところまで含めて読む必要がある。

### 28.2 直感: 練習用の直線と実際に走る道は違う
...
### 28.3 コードで見る: 最初のlossは同じである

第のLinear Path CFMと、1回目のRectified Flow（1-Rectified Flow）は、どちらも
...
つまり、最初のコード変更の中心はlossではなく、**軌道を保存し、直線性を評価し、次のReflowへ渡せるようにすること**である。

### 28.4 理論: marginal preserving property
...
### 28.5 理論と実用の接続: なぜ少ないステップと関係するのか
...
### 29.3.1 Rectified FlowとReflowの違い
...
複数のpathが同じ途中点で交差すると、その場所では異なる教師速度が混ざる。MSEが学ぶのはそれらの条件付き平均なので、学習される速度は各ペアの直線方向からずれ、生成軌道が曲がり得る。これは第10.4節で説明した「二乗誤差は平均を学ぶ」という性質の、Reflowにおける現れである。
...
## 30. Rectified Flowを実験として読む

> **この章で扱う概念**: Rectified Flow実験は、1回目の学習、Reflowによる再ペアリング、生成時のNFE別評価を区別し、直線性と少NFE生成を測る検証である。
...
#### 30.3.2 lossを変更する前に確認すること

標準のCFMでMSEを使うのは、単に実装が簡単だからではない。第10.4節で説明したように、MSEは同じ途中点へ来る条件付き速度の平均を学ぶ。この性質によって、ペアごとの教師速度から周辺速度場を学べる。
...
### 30.5 NFEとRectified Flow

Rectified FlowやReflowで軌道をまっすぐにしたい理由は、生成時のNFEに現れる。
...
とする評価が重要になる。ただし、1-stepだけが良くても、多stepで挙動が不安定なら、速度場としては扱いにくい可能性がある。NFEの一般的な定義とsolver間の公平な比較方法は、5.4節〜5.6節、第9部で扱う。
...
第7部のコードは、画像shapeを扱う速度場モデルの最小部品を理解するためのものである。dataset読み込み、長時間学習、重みの移動平均、複数GPU学習、画像生成評価までを含む完成済み画像生成pipelineではない。まず `model(t,x_t)` が画像と同じshapeの速度を返せるところまでを扱い、学習・生成の接続には第13章、第42章、第44章の共通パイプラインを使う。
...
変わるのは、$v_\theta(t,x_t)$ を表すニューラルネットワークである。
...
| 分布の一致 | generatedがtargetを覆うか、targetデータへ高い密度を割り当てるか | MMD, FID, NLL, precision/recall |
...
| NLL | targetデータへモデルが割り当てる確率密度 | targetデータの尤度が高い | targetデータの尤度が低い | 低いほどよい。data preprocessing、dequantization、次元正規化、密度計算用solverに依存する |
...
| KID | 実画像と生成画像の特徴分布のkernel MMD² | 0に近いほど特徴分布が近い | 大きいほど特徴分布が離れている | 不偏推定値は有限サンプルで負になる場合がある。kernel、部分集合サイズ、反復回数を明記する |
...
### 37.4 NLL: targetデータへ割り当てた密度を測る

NLL（負の対数尤度）は、生成サンプルの見た目ではなく、学習済みモデルが実際のtargetデータへどれだけ高い確率密度を割り当てたかを測る。

#### 直感: 真のデータ分布をどれだけ正確に再現できたか

生成モデルの目的は、観測されたデータを生み出した未知の真の分布 $p_{\mathrm{data}}$ を、モデル分布 $p_1$ で再現することである。NLLは、実データが現れる場所へモデルも高い密度を割り当てているかを調べ、**真のデータ分布をモデル分布がどれだけ正確に再現できているか**を定量評価する。

```text
真のデータ分布 p_data
  ↓ validation/testデータとして有限個だけ観測する
学習済みモデル分布 p_1
  ↓ 観測データへ割り当てた密度を測る
平均NLLが小さいほど、p_1がp_dataをよく再現している
```

たとえば真の分布に複数のmodeがあるのに、モデルが一部のmodeへほとんど密度を割り当てていなければ、そのmodeから来た実データのNLLが大きくなる。反対に、実データが多く現れる領域へモデルも適切に密度を割り当てていれば、平均NLLは小さくなる。このようにNLLは、生成例を目で比べるだけでは分かりにくい分布の再現度を一つの数値で比較できる。

ただし、真の分布 $p_{\mathrm{data}}$ の式そのものは分からないため、実際にはそこから得られた有限個のデータで平均を近似する。学習に使ったデータだけで測ると、分布を学んだのではなく個別データを記憶したモデルが有利になることがある。そのため、モデル間の比較には原則として学習に使っていないvalidation/testデータを使う。

targetデータ $x\sim p_{\mathrm{data}}$ に対する平均NLLは

$$
\operatorname{NLL}
=
-\mathbb{E}_{x\sim p_{\mathrm{data}}}
\left[\log p_1(x)\right]
$$

である。$p_1(x)$ は、Flow MatchingのODEが時刻1で定めるモデル分布の密度である。モデルがtargetデータへ高い密度を割り当てるほど $\log p_1(x)$ は大きくなり、その負号を取ったNLLは小さくなる。

期待値として見たNLLは、真の分布とモデル分布のcross entropyであり、
...
H(p_{\mathrm{data}})
+
D_{\mathrm{KL}}(p_{\mathrm{data}}\|p_1)
$$

と分解できる。同じdataset上でモデルを比較するとき、真の分布自身のentropy $H(p_{\mathrm{data}})$ は共通である。したがってNLLが低いモデルほど、真の分布からモデル分布へのKL divergenceが小さく、密度分布をより正確に再現していると解釈できる。

#### Flow MatchingのODEから密度を計算する
...
と動くとき、その点の対数密度はinstantaneous change-of-variables formula

$$
\frac{d}{dt}\log p_t(x_t)
=
-\nabla_x\cdot v_\theta(t,x_t)
$$

に従って変化する。$\nabla_x\cdot v_\theta$ は速度場の**発散**であり、各座標方向について「その方向の速度が、その座標に対してどれだけ変化するか」を足した量である。

$$
\nabla_x\cdot v_\theta(t,x)
=
\sum_{j=1}^{d}
\frac{\partial v_{\theta,j}(t,x)}{\partial x_j}.
$$

したがって、source点 $x_0$ からtarget点 $x_1$ までの軌道では
...
-
\int_0^1
\nabla_x\cdot v_\theta(t,x_t)\,dt
$$

となる。評価時にはtargetデータ $x_1$ からODEを逆向きに解いて対応するsource点 $x_0$ を求め、既知のsource密度 $p_0(x_0)$ と発散の積分から $\log p_1(x_1)$ を計算する。

直感的には、点群が広がる場所では密度が下がり、縮む場所では密度が上がる。その広がり・縮みを発散で追跡し、標準正規分布などの既知のsource密度からtarget側の密度を復元している。

#### 実験でNLLを比較するときの注意

- **同じtargetデータを使う**: 生成サンプルではなく、固定したvalidation/testデータのNLLを測る。
- **密度計算のsolverを固定する**: solver、許容誤差、time grid、NFEが違うと、モデルが同じでもNLLの数値積分誤差が変わる。
- **高精度の基準値を置く**: 十分細かいRK4や適応solverの値と比較し、モデルの密度誤差と粗いsolverの誤差を分ける。
- **発散計算のコストを記録する**: NLLではvelocityのforwardに加えて微分が必要であり、生成時のNFEだけでは計算量を表しきれない。
- **高次元では推定分散を見る**: 2DならJacobianの対角和を直接計算できるが、画像ではHutchinson trace estimatorなどの確率的近似を使うことが多い。
- **画像ではbits/dimの定義を固定する**: pixelのdequantization、値域、VAE latentで測るかpixelで測るかを明記する。

NLLは密度モデルとしての評価には重要だが、知覚品質と常に一致するわけではない。低いNLLでも生成画像の見た目がよいとは限らず、反対にFIDやKIDが良いモデルが最良のNLLを持つとも限らない。training loss、NLL、生成標本に基づくFID・KID・MMD、計算コストを別々の軸として報告する。

`evaluate_minimal_2d.py` は生成標本に基づくMMD、mode coverage、endpoint errorを扱う。NLLは [evaluate_solver_study_2d.py](src/evaluate_solver_study_2d.py) から評価でき、状態と対数密度補正を同時に積分するaugmented ODEと発散計算は [likelihood.py](src/fm_minimal/likelihood.py) に実装している。ファイル名の `likelihood` は「尤度」という意味である。
...
```text
_outputs/minimal_2d_evaluation.csv
figures/minimal_2d_solver_comparison.png
```
...
表の値をNFEに対して可視化した結果を図6に示す。

![EulerとHeunのNFE別評価](figures/minimal_2d_solver_comparison.png)

*図6　EulerとHeunの生成品質を同一NFEで比較した結果*

**図6の読み方**: 左は分布全体の差を表すbiased MMD²で、低いほどよい。中央は8 modeのうち生成できた割合で、高いほどよい。右は生成点からtarget点群までの最近傍距離で、低いほどよい。横軸はstep数ではなくNFEなので、たとえばEuler 16 stepsとHeun 8 stepsが同じNFE 16で並ぶ。
...
ここでいう **Inception特徴** を先に説明する。画像をpixel値のまま比較すると、少しの位置ずれや明るさの違いだけでも大きな差になり、画像の内容が似ているかを捉えにくい。そこで、ImageNetの画像分類用に学習されたInception-v3へ画像を入力し、分類結果そのものではなく、出力直前の中間層からベクトルを取り出す。このベクトルがInception特徴である。一般的なFID実装では、各画像を2048次元の特徴ベクトルへ変換する。

```text
画像
  -> 同じInception-v3へ入力
  -> 画像の形・色・物体などを要約した特徴ベクトル
  -> 実画像群と生成画像群でベクトルの分布を比較
```

したがってFIDが比較するのはpixel分布そのものではなく、**Inception-v3から見た画像内容の特徴分布**である。実画像が10,000枚あれば10,000本の特徴ベクトルが得られ、生成画像についても同様にベクトルの集まりが得られる。その二つの集まりを、それぞれ平均と共分散で要約する。

実画像の特徴ベクトルの平均と共分散を $(\mu_r,\Sigma_r)$、生成画像側を $(\mu_g,\Sigma_g)$ とすると、FIDは
...
で表される。第1項は特徴分布の**中心のずれ**、第2項は特徴のばらつき方や相関を表す**共分散のずれ**を測る。$\operatorname{Tr}$ は行列の対角成分の和、$(\Sigma_r\Sigma_g)^{1/2}$ は共分散を比較するための行列平方根である。

この式では、実画像から得た特徴ベクトル群と生成画像から得た特徴ベクトル群を、それぞれ一つの多変量Gaussianで近似している。つまり、各分布を「特徴ベクトルの平均的な位置」と「各特徴の広がり・相関」だけで要約して比較する。平均と共分散が近ければFIDは低くなるが、それより細かな分布形状の違いをすべて識別できるわけではない。
...
### 39.2 KID: kernelで特徴分布を比較する

KID（Kernel Inception Distance）も、実画像と生成画像をInception特徴へ変換してから分布差を測る。ただし、平均と共分散だけを使うFIDとは異なり、KIDはkernelを用いたMMD²として定義される。

実画像の特徴を $r_1,\ldots,r_m$、生成画像の特徴を $g_1,\ldots,g_n$ とする。不偏推定量は
...
\frac{1}{n(n-1)}\sum_{i\ne j}k(g_i,g_j)
-
\frac{2}{mn}\sum_{i=1}^{m}\sum_{j=1}^{n}k(r_i,g_j)
$$

と書ける。第1項は実画像同士、第2項は生成画像同士の特徴の似方を測り、第3項で実画像と生成画像の間の似方を差し引く。二つの分布が近ければ、分布内の類似度と分布間の類似度が近づき、KIDは0に近づく。

よく使われるのは3次の多項式kernel
...
である。$d$ は特徴次元であり、$x^\top y$ は二つの特徴ベクトルの内積である。

KIDの母集団上のMMD²は非負だが、不偏推定量は有限サンプルで0より小さくなることがある。これは「生成分布が実分布より良い」という意味ではなく、標本による推定の揺れである。実験では、使用した特徴抽出器、kernel、サンプル数、部分集合の大きさ、部分集合を取り直した回数を報告し、平均とばらつきを見る。

### 39.3 FIDとKIDを実用上どう使い分けるか

FIDとKIDはどちらも低いほど、実画像と生成画像の特徴分布が近いと読む。ただし、推定方法が異なるため、実験の目的とサンプル数に応じて使い分ける。

| 状況 | 主に使う指標 | 理由 |
|---|---|---|
| 既存論文や公開benchmarkと比較したい | FID | 広く報告されており、同じ評価protocolなら比較しやすい |
| 評価サンプル数が比較的少ない | KIDを併用 | FIDには有限標本でのbiasがあり、KIDはMMD²の不偏推定量を使える |
| 実験間の小さな差を調べたい | FIDとKIDを両方報告 | 指標固有の仮定や推定の揺れによる見かけの改善を疑える |
| 評価値の不確かさも示したい | KIDの部分集合反復 | 部分集合ごとの平均と標準偏差を記録しやすい |

実務では、まず既存研究との比較軸としてFIDを計算し、可能ならKIDも併記するのが分かりやすい。FIDだけが改善しKIDが改善しない場合は、Gaussian近似、サンプル数、推定の揺れの影響を疑う。反対にKIDだけが改善した場合も、部分集合の選び方や分散を確認し、単一の値だけで優劣を決めない。

どちらを使う場合も、**同じ実画像集合、同じ生成枚数、同じInception-v3の重み、同じresize・正規化、同じ評価実装**を固定する。特徴抽出器や前処理が違うFID同士、KID同士は、名前が同じでも直接比較できない。また、両指標は画像の特徴分布全体を要約する値であり、個々の画像の破綻理由やpromptとの一致を説明しない。生成例、precision/recall、条件付き生成ならprompt整合性の指標も合わせて見る。

### 39.4 precision/recallとcoverage
...
Rectified Flowなど少ステップ生成を目指す手法では、少ないNFEでの品質が重要になる。評価表では、次のようにNFEを分けて出す。
...
- NLLはtargetデータへ割り当てた密度を測り、生成標本に基づくFID、KID、MMDとは異なる評価軸である。
- 2D toyでは、MMD、mode coverage、endpoint errorを組み合わせる。
- 画像生成では、FIDとKIDを単独で絶対視せず、NFE、solver、サンプル数、特徴抽出器、前処理を明記する。
...
| NLL | CNFの尤度・発散評価 | 逆向きODE、発散計算、solver tolerance、bits/dimの定義 |
| FID | evaluation script | サンプル数、解像度、前処理 |
| KID | evaluation script | kernel、部分集合サイズ、反復回数、特徴抽出器 |
...
> **この章で扱う概念**: 第1部で整理したsampler、solver、time grid、NFEを使い、solverを変えたときの離散化誤差と計算量を比較する。

samplerの定義、Euler生成、sampler・solver・time grid・NFEの役割分担については5.1節〜5.6節を参照してほしい。この部では、学習済みモデルと初期乱数を同じに保ち、solverとNFEだけを変える。

この部でいう「一般的」は、すべてのFlow Matching論文が必ず行うという意味ではない。Flow Matching、Rectified Flow、Stochastic Interpolants、少step生成、continuous normalizing flowの代表的研究で、目的に応じて反復して使われる評価かどうかを表す。

| 比較実験・原則 | Flow Matching評価での一般性 | 主な用途 |
|---|---|---|
| 比較対象以外を固定する | **一般的な実験設計原則** | solver以外の影響を除く |
| 同じNFEでsolverを比較する | **solver比較では一般的** | model呼び出し回数をそろえる |
| NFE sweep | **高速sampling研究では一般的** | 少ない計算量から品質が収束する過程を見る |
| 同じstep数でsolverを比較する | **補助的な診断** | 1 stepの高次化による差を見る。計算量は公平ではない |
| 品質–計算量曲線 | **高速化を主張する研究では一般的** | 品質と計算予算のPareto frontierを見る |
| NLLのsolver精度依存性 | **尤度評価では重要だが、sample品質中心のFMでは限定的** | 密度計算の数値誤差を調べる |

一般性の分類は、Flow Matching、Rectified Flow、SiTなどの生成評価、DPM-SolverやEDMのsampling評価、FFJORDなどのcontinuous normalizing flowにおける尤度評価を対応づけたものである。

この部の4実験は [evaluate_solver_study_2d.py](src/evaluate_solver_study_2d.py) でまとめて実行できる。NLL計算の状態・対数密度同時積分と2Dの厳密な発散計算は [likelihood.py](src/fm_minimal/likelihood.py) に分離している。ここでも `likelihood` は尤度を指す。
...
```text
_outputs/solver_study/solver_quality_compute.csv
_outputs/solver_study/solver_quality_compute.png
_outputs/solver_study/solver_nll_sensitivity.csv
_outputs/solver_study/solver_nll_sensitivity.png
```

### 42.1 この部で固定するものと変えるもの

> **一般性: 一般的な実験設計原則。** Flow Matchingに限らず、solverやsamplerを比較する実験では必須である。

| 固定するもの | 変えるもの | 観察するもの |
|---|---|---|
| checkpoint、初期乱数、time gridの区間、評価サンプル数 | solver、step数 | 生成品質、NFE、実行時間 |

複数要素を同時に変えると、品質差がsolverによるものか学習済みモデルによるものか判断できない。

### 42.2 同じNFE予算で比較する

> **一般性: solver比較では一般的。** solverごとに1 step当たりのmodel呼び出し回数が違う場合、同じ計算予算へ近づけるための主比較として使う。
...
seed 0、生成・target各2048サンプルでNFE 16、32、64を比較した結果を示す。

| NFE | solver | steps | MMD² | endpoint error | 実時間の中央値 |
|---:|---|---:|---:|---:|---:|
| 16 | Euler | 16 | 0.002767 | 0.04453 | 33.5 ms |
| 16 | Heun | 8 | 0.002148 | 0.03668 | 43.1 ms |
| 16 | RK4 | 4 | 0.002055 | 0.03284 | 29.5 ms |
| 32 | Euler | 32 | 0.002143 | 0.03703 | 65.1 ms |
| 32 | Heun | 16 | 0.002002 | 0.03170 | 88.6 ms |
| 32 | RK4 | 8 | 0.002018 | 0.03042 | 87.0 ms |
| 64 | Euler | 64 | 0.002104 | 0.03484 | 109.5 ms |
| 64 | Heun | 32 | 0.001898 | 0.03065 | 162.8 ms |
| 64 | RK4 | 16 | 0.001910 | 0.02972 | 139.8 ms |

同一NFEでは、HeunとRK4がEulerより小さいendpoint errorを示した。MMD²ではNFE 32と64でHeunがわずかによく、endpoint errorではRK4がわずかによい。指標によって順位が一致しないため、単一指標だけでsolverを選ばない。

`evaluate_solver_study_2d.py` は、同じNFE予算からsolverごとのstep数を自動計算し、`comparison=same_nfe` として保存する。
...
NFE予算が1 step当たりの呼び出し回数で割り切れない場合は、予算を超えない最大の整数step数を使う。CSVには要求した予算と実際に使ったNFEを別々に保存する。

### 42.3 NFE sweep: NFE=1から128まで品質曲線を比較する

> **一般性: 高速samplingや少step生成の研究では一般的。** 単一NFEでの勝敗ではなく、計算予算を増やしたときの品質の改善と飽和を見る。

NFE sweepでは、NFEを段階的に増やし、各solverの生成品質を曲線として比較する。基本の予算は

```text
1, 2, 4, 8, 16, 32, 64, 128
```

とする。ただしHeunは1 stepに2 NFE、RK4は1 stepに4 NFEを使うため、すべてのsolverがNFE 1を実現できるわけではない。実現できない点を無理に補間せず、solverごとの**実NFE**を横軸に記録する。
...
scriptは1 step分のNFEに満たない条件を自動的に欠測とする。そのためNFE 1はEulerだけ、NFE 2はEulerとHeun、NFE 4以降は三つのsolverが評価される。

2D toyではMMD²、mode coverage、endpoint errorを縦軸にする。画像モデルでは第8部で説明したFID、KID、precision/recallを使う。次の点を読む。

- 低NFEで最初に品質が立ち上がるsolverはどれか。
- NFEを増やしたとき、どこで改善が飽和するか。
- 高NFEでもsolver間の差が残るか。
- 一つの指標だけ改善し、coverageなどが悪化していないか。

NFE sweepは、最良値だけでなく曲線全体を報告する。同じNFEで点が存在しない場合は、線形補間した値を主結果にせず、観測した点をそのまま示す。

seed 0、生成・target各2048サンプルで実行した図7では、EulerのNFE 1はmode coverageが0、NFE 2以降は三つのsolverの観測点すべてで1になった。したがって、このcheckpointでは「modeを一つでも落としているか」は極低NFEの失敗検出には使えるが、NFE 2以降のsolver差は判別できない。

MMD²とendpoint errorは、どのsolverでもおおむねNFE 8〜16までに急速に改善し、その後の改善は小さい。NFE 64から128ではHeunとRK4の値がわずかに悪化する箇所もある。この小さな反転だけから高NFEが有害だとは結論せず、有限標本による評価の揺れ、学習済み速度場そのものの誤差、solver誤差の大小関係を複数seedで確認する必要がある。

### 42.4 同一step数比較と同一NFE比較を分ける

> **一般性: 同一NFE比較は一般的。同一step数比較は補助的な診断。** 二つは答える質問が違うため、別の表として報告する。

同じstep数で比べると、HeunやRK4は1区間をどれだけ丁寧に積分できるかを見られる。しかし、高次solverほどmodelを多く呼ぶため、計算量をそろえた比較にはならない。
...
python .\src\evaluate_solver_study_2d.py `
  ...
  --solvers euler heun rk4 `
  --same-steps 8 16 32 64 `
  --nfe-budgets 16 32 64 128 `
  --skip-nll `
  --seed 0
```

| 比較 | 固定する値 | 答える質問 | 主結果としての位置づけ |
|---|---|---|---|
| 同一step数 | time gridの区間数 | 1 stepを高次化すると誤差が減るか | 補助的 |
| 同一NFE | model呼び出し回数 | 同じmodel計算予算でどのsolverがよいか | solver比較の主結果 |

たとえば32 stepsでは、EulerはNFE 32、HeunはNFE 64、RK4はNFE 128である。この結果だけでRK4が優れていると結論すると、solverの次数と計算量の増加を分離できない。一方、同一NFE比較だけでは「1 stepを丁寧にした効果」が見えにくい。両方を並べることで、精度向上が高次更新によるのか、modelを多く呼んだためなのかを判断する。

seed 0、生成・target各2048サンプルで実行した結果から、16 stepsとNFE 64を抜き出すと次のようになる。

| 比較方法 | solver | steps | NFE | MMD² | endpoint error |
|---|---|---:|---:|---:|---:|
| 同一step数 | Euler | 16 | 16 | 0.002767 | 0.04453 |
| 同一step数 | Heun | 16 | 32 | 0.002002 | 0.03170 |
| 同一step数 | RK4 | 16 | 64 | 0.001910 | 0.02972 |
| 同一NFE | Euler | 64 | 64 | 0.002104 | 0.03484 |
| 同一NFE | Heun | 32 | 64 | 0.001898 | 0.03065 |
| 同一NFE | RK4 | 16 | 64 | 0.001910 | 0.02972 |

同一16 stepsでは高次solverほどMMD²とendpoint errorが改善しているが、NFEもEuler 16、Heun 32、RK4 64へ増えている。同一NFE 64にそろえると差は小さくなり、MMD²はHeun、endpoint errorはRK4がわずかによい。したがって、この結果から「RK4が常に最良」とは言えず、評価指標と計算予算を明示する必要がある。なお、ここに示した6条件ではcoverageがすべて1であり、solver差を判別する指標としては飽和している。

### 42.5 品質–計算量曲線とPareto frontier

> **一般性: 高速化・少step化を主張する研究では一般的。** 最高品質だけでなく、その品質を得るための計算予算を同時に示す。

NFE sweepの結果を、横軸に計算量、縦軸に品質を置いた曲線として読む。最低限の横軸はNFEである。実用評価ではwall-clock time、throughput、peak memoryも併記する。

| 横軸候補 | 何を表すか | 注意点 |
|---|---|---|
| NFE | velocity modelの評価回数 | solver内部処理や発散計算のコストは含まない |
| wall-clock time | 実際の生成時間 | hardware、batch size、実装最適化に依存する |
| throughput | 1秒当たりの生成数 | batch sizeと並列性を固定する |
| peak memory | 最大メモリ使用量 | 高次solverが中間状態を保持する影響を見る |

品質指標は、低いほどよいMMD²、endpoint error、FID、KID、NLLと、高いほどよいcoverage、precision、recallで上下の向きが違う。グラフの凡例か軸名へ改善方向を明記する。

**Pareto frontier** は、「他の設定より計算量が多いのに品質も悪い」という設定を除き、品質と計算量のどちらかを改善するには他方を犠牲にする点だけを結んだ境界である。solverの優劣は単一の最良点ではなく、低予算から高予算までどの範囲でPareto frontierに残るかを見る。

`evaluate_solver_study_2d.py` はNFEに対するMMD²、coverage、endpoint error、wall-clock timeの曲線を作る。CSVにはthroughputとpeak追加memoryも保存する。実時間はwarm-up後に複数回測り、CUDAでは同期してから計時する。`--warmup` と `--timing-repeats` を全条件で固定する。

seed 0、生成・target各2048サンプル、既定のMMD bandwidth 0.5、coverage半径0.35で実行した例を図7に示す。

![solverごとの品質–計算量曲線](figures/solver_quality_compute.png)

*図7　Euler・Heun・RK4のNFE sweepと品質–計算量曲線*

**図7の読み方**: 左上のMMD²と左下のendpoint errorは低いほどよく、右上のcoverageは高いほどよい。EulerのNFE 1では8 modeを覆えないが、NFE 2以降はcoverageが1になり、coverageだけではsolver差を判別できなくなる。MMD²とendpoint errorはNFE 8〜16まで急速に改善し、その後は改善量が小さくなるため、このcheckpointではその付近から計算量に対する収益が逓減している。右下はwarm-up後の複数回計測の中央値である。NFE 32での実時間はEuler約65.1 ms、Heun約88.6 ms、RK4約87.0 msであり、同じNFEでも実時間は一致しない。さらに、この実行ではHeunとRK4の実時間順位も単純にsolverの次数順ではない。NFEはmodel評価回数を表す比較しやすい予算だが、solver固有の演算や実装上のoverheadまでは表さない。ただし実時間の順位と差はhardware、PyTorchのthread設定、batch sizeに依存するため、別環境へそのまま一般化しない。

### 42.6 NLLはsolver精度でどの程度変動するか

> **一般性: 尤度を報告するcontinuous normalizing flowでは重要だが、FIDなどsample品質だけを扱うFlow Matching研究では限定的。** NLLを主要指標にする場合の数値検証として行う。

第37.4節で説明したように、NLLはtargetデータからsourceへ状態を逆向きに積分し、同時に速度場の発散を積分して計算する。したがって、同じ学習済みモデルでもsolver、step数、適応solverの許容誤差によって推定NLLが変わり得る。

実験では、固定したvalidation/testデータに対して次を比較する。

| 設定 | solver | stepsまたはtolerance | 実NFE | NLL | 基準NLLとの差 | 実行時間 |
|---|---|---|---:|---:|---:|---:|
| 基準 | 高精度RK4または適応solver | 十分細かい | 記録する |  | 0 |  |
| A | Euler | 16, 32, 64, 128 |  |  |  |  |
| B | Heun | 8, 16, 32, 64 |  |  |  |  |
| C | RK4 | 4, 8, 16, 32 |  |  |  |  |

ここで見るのは、どのsolverが最小NLLを偶然出すかではなく、計算予算を増やしたときにNLL推定値が高精度の基準値へ収束するかである。粗い積分でNLLが低く見えても、それはモデルが改善したのではなく数値誤差の可能性がある。

NLL評価では、velocity modelの呼び出しに加えてJacobian traceまたはHutchinson estimatorによる発散計算が必要になる。そのため、生成時のNFEだけでなく、発散評価回数、trace estimatorの標本数、wall-clock timeも記録する。画像やlatentのNLLでは、dequantization、bits/dimの定義、VAEを含むかも固定する。

教材実装では、2DなのでJacobianの対角和を厳密に計算する。状態と対数密度補正を同じEuler・Heun・RK4で逆向きに積分し、高精度RK4を基準としてNLL差を保存する。

```powershell
python .\src\evaluate_solver_study_2d.py `
  --checkpoint .\_outputs\minimal_2d.pt `
  --nll-samples 64 `
  --nll-nfe-budgets 4 8 16 32 64 `
  --nll-reference-solver rk4 `
  --nll-reference-steps 128 `
  --out-dir .\_outputs\solver_study `
  --seed 0
```

`solver_nll_sensitivity.csv` にはNLL、基準NLLとの差、実NFE、実行時間を保存する。画像へ拡張すると厳密な発散は高コストになるため、Hutchinson estimatorなどへの置き換えが必要である。

64個のtarget標本を使い、RK4 128 steps、すなわちNFE 512の推定値を基準とした結果を図8に示す。

![solver精度によるNLL推定値の変化](figures/solver_nll_sensitivity.png)

*図8　Euler・Heun・RK4によるNLL推定値と高精度基準からの絶対誤差*

**図8の読み方**: 左の破線は基準NLL 0.2893 natsである。低NFEではNLL推定値が大きく上下し、値が小さいからといってモデルの尤度が本当に改善したとは言えない。右を見ると、NFE 64で基準値との差はEuler 0.1278、Heun 0.0181、RK4 0.0131まで縮まり、高次solverの方が少ないstepで基準へ近づいている。このNLLは連続密度に対する値なので、負の推定値自体は異常を意味しない。また、64標本・単一seedの数値であるため、最終的な研究結果では標本数とseedを増やして平均とばらつきを報告する。

## 43. 第9部のまとめと公式実装への入口

### 43.1 確認事項

- checkpoint、初期乱数、評価サンプル数を固定し、solverとstep数だけを変える。
- Euler、Heun、RK4は1 stepあたりのモデル呼び出し回数が違うため、step数だけでは公平に比較できない。
- solver比較では総NFEをそろえ、生成品質と実行時間を測る。
- NFE sweepでは1から128までの品質曲線を測り、改善の立ち上がりと飽和を見る。
- 同一step数比較は補助診断、同一NFE比較は計算予算をそろえた主比較として分ける。
- 高速化を主張するときは、品質–計算量曲線とPareto frontierを報告する。
- NLLのsolver感度は尤度評価では重要だが、sample品質中心の研究では必須ではない。
- 十分大きなNFEでも品質が低い場合は、solverより学習済み速度場の誤差を疑う。
...
| NFE sweep | sampling/evaluation config | 複数NFEの品質曲線を報告しているか |
| 品質–計算量 | benchmark table / plot | NFE、実時間、品質を同時に示しているか |
| NLL感度 | CNFの尤度評価 | divergence、solver tolerance、基準解との差 |

公式実装の評価表では、solver名、step数、NFEを同時に確認する。少NFEの主張では、ここが実験の中心になる。

# 第10部 実装に必要なパーツを一枚の設計図にまとめる

## 44. Flow Matching実装の全体像
...
### 44.1 学習と生成は別のパイプラインである

学習と生成で共有する部品と、それぞれに固有の部品を図9に示す。

![Flow Matching実装の全体像](figures/implementation_parts_overview.png)

*図9　Flow Matchingにおける学習パイプラインと生成パイプライン*

**図9の読み方**: 上段は学習である。$x_0, x_1$ のペアからpathが途中点 $x_t$ と教師速度 $u_t$ を作り、モデルの予測と比較する。下段は生成である。教師速度もtargetデータも使わず、学習済みモデルをODE solverから繰り返し呼ぶ。紫色のモデルが両方に現れることが重要で、学習時に作った速度場を生成時に積分している。
...
数式では、`path.sample` と `path.velocity` が第4.2節で導入した
...
に対応する。モデルの入出力は第12.1節で導入した

$$
v_\theta : [0,1] \times \mathbb{R}^{d} \to \mathbb{R}^{d}
$$

に対応し、生成時には第1.3節と第5.2節で説明したODE

$$
\frac{dX_t}{dt}=v_\theta(t,X_t)
$$

として使う。したがって44.3節の式は新しい定義ではなく、既出の三つの部品を実装インターフェース順に並べ直したものである。

`path.velocity` の出力と `model` の出力が同じ意味・shapeでなければ、MSEを計算できても学習内容は壊れる。shapeが一致することだけでなく、両方が同じ時間向きの速度を表すことを確認する。

### 44.4 tensor shapeの契約
...
| 調べたい部品 | 戻る章 |
|---|---|
| Linear pathとCFM loss | 第2章、第4章、第10章 |
| モデル・学習ループ・checkpoint | 第12章、第13章 |
| Diffusion schedulerと予測target | 第17章〜第21章 |
| couplingとOT-CFM | 第24章〜第26章 |
| Rectified FlowとReflow | 第28章〜第30章 |
| Euler、Heun、NFEの比較実験 | 第42章 |
| U-NetとDiT | 第32章〜第35章 |
| 評価とablation | 第37章〜第40章 |

本章は、前章までの代わりではなく、実装時に机の横へ置く設計図である。「どの式を変えたのか」「どのPython部品を変えるのか」「比較時に何を固定するのか」の三点を、常に同じ行で考える。

## 45. 第10部のまとめと公式実装への入口

### 45.1 確認事項

- 学習pipelineはdata、coupling、path、time sampler、model、loss、optimizerを接続する。
- 生成pipelineはsource、学習済みmodel、time grid、solver、evaluatorを接続する。
- 新しい手法は、どの部品とインターフェースを変えるかとして実装する。
- 比較実験では、変更しない部品、seed、NFE、評価条件を固定する。

### 45.2 公式実装の読み方

公式実装ではファイル名より、各オブジェクトの入力と出力を追う。Meta `flow_matching` ではpathとsolver、TorchCFMではFlowMatcher、Diffusers系ではmodel・scheduler・pipelineの契約を確認する。

| 教材の部品 | 公式実装で探すもの | 最初に確認すること |
|---|---|---|
| path | path / scheduler / FlowMatcher | $x_t$ と教師速度をどこで作るか |
| velocity model | model / transformer / U-Net | 入出力shapeと予測target |
| objective | training step / loss | model出力を何と比較するか |
| solver | solver / scheduler step / sampling loop | 時間向きと1 step当たりのNFE |
| evaluator | evaluation script | metric、前処理、サンプル数 |

# 第11部 学習済みFlow Matchingモデルを改造して使う

## 46. 公開済みモデル、ファインチューニング、重みの再利用
...
### 46.1 まず「Flow Matching系」の範囲を決める
...
したがって「どれもDiTだから重みを流用できる」とは限らない。第21章の予測target、第42章のsampler、第44章の部品契約を、checkpoint単位でもう一度確認する必要がある。
...
### 46.4 text encoder、VAE、Transformerを分けて考える

text-to-image pipelineは一つのニューラルネットワークではない。
...
### 46.5 Flow Matchingとしてfine-tuning lossを読む
...
この順序なら、「学習が失敗した」「読み込みが失敗した」「solver設定が違う」「ライセンス上配布できない」を別々に診断できる。学習済みモデルの利用は、第44章の部品設計を実際の大規模pipelineへ拡張したものだと考えるとよい。

## 47. 第11部のまとめと公式実装への入口
...
公開済みモデルを改造するときは、モデル定義だけでなく、pipeline、VAE、text encoder、scheduler、adapter読込処理を一組として追う。

| 目的 | 公式実装で見る場所 | 確認すること |
|---|---|---|
| base modelの復元 | model card / pipeline config | model ID、revision、prediction type、VAE |
| LoRA学習 | Diffusers training examples / PEFT | target modules、rank、alpha、保存形式 |
| adapterの利用 | adapter loading API | base modelとの互換性、scale、merge前後の一致 |
| 配布 | model card / license | base model、dataset、派生weightの利用条件 |
...
- Will Grathwohl et al., ["FFJORD: Free-form Continuous Dynamics for Scalable Reversible Generative Models"](https://arxiv.org/abs/1810.01367), ICLR 2019. continuous normalizing flowのinstantaneous change-of-variables、発散推定、NLL計算の基礎資料。
- Cheng Lu et al., ["DPM-Solver: A Fast ODE Solver for Diffusion Probabilistic Model Sampling in Around 10 Steps"](https://arxiv.org/abs/2206.00927), NeurIPS 2022. NFEをそろえた高速sampling評価と、少stepでの品質比較を考えるための代表的なsolver研究。
- Tero Karras et al., ["Elucidating the Design Space of Diffusion-Based Generative Models"](https://arxiv.org/abs/2206.00364), NeurIPS 2022. sampler設定、NFE、FID、計算量を分けて評価する実験設計の参考資料。
...
- Martin Heusel et al., ["GANs Trained by a Two Time-Scale Update Rule Converge to a Local Nash Equilibrium"](https://arxiv.org/abs/1706.08500), NeurIPS 2017. Fréchet Inception Distance（FID）を導入した論文。
- Mikołaj Bińkowski et al., ["Demystifying MMD GANs"](https://arxiv.org/abs/1801.01401), ICLR 2018. Kernel Inception Distance（KID）とその不偏推定を導入した論文。
...
- TorchCFM documentation, [atong01/conditional-flow-matching](https://github.com/atong01/conditional-flow-matching). CFM variantsをFlowMatcherクラスとして読むための実装資料。
...
- Itai Gat et al., ["Discrete Flow Matching"](https://arxiv.org/abs/2407.15595), 2024. 連続空間のFlow Matchingを離散データへ拡張する研究資料。
- Aram-Alexandre Pooladian et al., ["Multisample Flow Matching"](https://proceedings.mlr.press/v202/pooladian23a.html), ICML 2023. ミニバッチ内の複数標本とcouplingを使って輸送を改善する観点の参考資料。
- Nikita Kornilov et al., ["Optimal Flow Matching: Learning Straight Trajectories in Just One Step"](https://proceedings.neurips.cc/paper_files/paper/2024/hash/bc8f76d9caadd48f77025b1c889d2e2d-Abstract-Conference.html), NeurIPS 2024. 直線的な軌道と少ステップ生成を比較するための発展資料。
- Xixi Hu et al., ["Improving Rectified Flow with Boundary Conditions"](https://openaccess.thecvf.com/content/ICCV2025/html/Hu_Improving_Rectified_Flow_with_Boundary_Conditions_ICCV_2025_paper.html), ICCV 2025. Rectified Flowの境界付近の挙動と改善方法を扱う資料。

### 公式実装・実装補助資料

- Meta, [facebookresearch/flow_matching](https://github.com/facebookresearch/flow_matching). path、solver、continuous/discrete exampleの公式PyTorch実装。
- Meta, [Flow Matching examples](https://github.com/facebookresearch/flow_matching/tree/main/examples). 2D continuous exampleを含む、教材コードから公式実装へ移るための参照先。
- Xingchao Liu et al., [gnobitab/RectifiedFlow](https://github.com/gnobitab/RectifiedFlow). Rectified Flowのtrain、eval、reflowを確認するための公式実装。
- Sangyun Lee et al., [sangyun884/rfpp](https://github.com/sangyun884/rfpp). "Improving the Training of Rectified Flows" に対応する参考実装。
- Jonathan Ho et al., [hojonathanho/diffusion](https://github.com/hojonathanho/diffusion). DDPMの公式実装。
- Hugging Face, [Diffusers](https://github.com/huggingface/diffusers). scheduler、pipeline、Flow Matching系モデル、学習・推論コードの実装資料。
- [Diffusion Meets Flow Matching](https://diffusionflow.github.io/). DiffusionとFlow Matchingの時間向き、予測対象、ODE/SDE対応を対話的に確認する資料。
- MIT 6.S184, [Flow Matching and Diffusion Models course site](https://diffusion.csail.mit.edu/). 講義ノートと関連教材の入口。

### 直感説明・可視化・動画の参考資料

- ["Rectified Flow: Straight is Fast"](https://rectifiedflow.github.io/blog/2024/intro/), Let Us Flow Together. Rectified Flow、直線軌道、Reflowの直感説明を検討するために使用した。
- ["Curved × Curved = Straight: DDIM is Straight RF"](https://rectifiedflow.github.io/blog/2024/DDIM-and-natural-euler/), Let Us Flow Together. DDIMとRectified Flowの軌道を比較する補助資料。
- ["Interpolations: All Flows are One Flow"](https://rectifiedflow.github.io/blog/2024/interpolation/), Let Us Flow Together. interpolationとpath設計の直感を確認する補助資料。
- ["Flow to Diffusion: Langevin is a Guardrail"](https://rectifiedflow.github.io/blog/2024/diffusion/), Let Us Flow Together. Flow、Diffusion、Langevin dynamicsの関係を確認する補助資料。
- ["A Visual Dive into Conditional Flow Matching"](https://dl.heeere.com/conditional-flow-matching/blog/conditional-flow-matching/), ICLR Blogposts 2025. CFMの条件付き速度、平均速度場、直感図解に役立つ解説。
- Peter Holderrieth, ["Flow Matching: A visual introduction"](https://peterroelants.github.io/posts/flow_matching_intro/). 粒子、分布、速度場、ODE生成を視覚的に説明する入門記事。
- ["Flow Matching for Generative Modeling (Paper Explained)"](https://www.youtube.com/watch?v=7NNxK3CqaDk). Flow Matching論文の解説動画およびその字幕transcript。生成モデル未経験者向けの説明粒度を検討するために使用した。
- [Zenn「Flow Matchingのエッセンス」](https://zenn.dev/doctorin/articles/flow-matching). 日本語での速度場、輸送、学習と生成の説明を点検するために使用した。
- [Zenn「Optimal Transport Conditional Flow Matching」](https://zenn.dev/fusic/articles/ml-flow-matching). OT-CFMの日本語説明と用語選択を確認するために使用した。
- [Qiita「Flow Models・Flow Matching・Diffusion Models 学習メモ」](https://qiita.com/kanta_yamaoka/items/b12ae01efd0ae1f56fed). 日本語でのFlow MatchingとDiffusionの対比を確認する補助資料。
- [Docswell「Conditional Flow Matching輪読資料」](https://www.docswell.com/s/DeepLearning2023/5VVRDV-2024-04-18-151949). CFM、OT-CFM、条件付き経路の日本語説明を確認する補助資料。
- [Zenn「Flowベース生成モデルの調査」](https://zenn.dev/d2c_mtech_blog/articles/6bd54c0db73a8e). Flow Matchingを周辺のflow-based generative modelsと比較するために使用した日本語Web資料。
