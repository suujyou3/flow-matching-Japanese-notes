...
$\operatorname{Beta}(\alpha,\beta)$ は、密度全体の面積を1にするための定数である。time samplerを理解する段階では、この定数を自分で計算する必要はない。重要なのは、$t$ と `1-t` の指数が分布の形を変えることである。
...
したがって、$\operatorname{Beta}(2,2)$ は中央重視、$\operatorname{Beta}(0.5,0.5)$ は両端重視になる。もし $\alpha$ と $\beta$ を異なる値にすれば、source側とtarget側のどちらか一方へ偏らせることもできるが、最初の教材コードでは比較を単純にするため左右対称だけを扱う。
