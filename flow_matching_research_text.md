...
5. **第5部から第7部**: coupling、Rectified Flow、solverを研究上の設計変数として学ぶ。
6. **第8部と第9部**: 画像モデルへ広げ、評価とablationを組む。
7. **第10部**: 全部品の接続を一枚の設計図で復習する。
8. **第11部（必要になってから読む）**: 公開済みモデルのfine-tuning、重みの再利用、互換性、ライセンスを扱う。最小モデルを自作するだけなら初読では飛ばしてよい。
...
この先の章では、これらを一つずつ数式とコードへ落としていく。今は用語を暗記する必要はない。「ペアを作る → 練習経路と正解矢印を作る → 矢印の地図を学ぶ → 新しいノイズを地図に沿って歩かせる」という関係だけをつかめばよい。詳しい実装上の接続は38.1節と図7で改めて整理する。
...
### 4.5 Linear pathと1-Rectified Flowの関係

4.3節のLinear path CFMと、1回目のRectified Flow（1-Rectified Flow）は、どちらも

$$
x_t=(1-t)x_0+tx_1,\qquad u_t=x_1-x_0
$$

を使った速度回帰である。したがって、最初の学習lossを読む段階では同じコードとして理解できる。

違いは、その後に何を目標として実験するかにある。Rectified Flowでは、学習済み速度場が作る実際の生成軌道を測り、Reflowでペアを作り直して、少ないODEステップでも追いやすい直線的な軌道を目指す。第6部では、この追加部分だけを扱う。
...
この例では、Eulerは $32\times1$、Heunは $16\times2$、RK4は $8\times4$ なので、すべて総NFEが32である。公平な比較では、このようにモデルを呼ぶ総回数をそろえ、生成品質と実行時間を測る。
...
### 5.5 samplerを構成するsolver、time grid、NFE
...
したがってsolverの公平な比較では、step数ではなく総NFEをそろえる。たとえばEuler 32 steps、Heun 16 steps、RK4 8 stepsはいずれもNFE 32である。第7部では、この共通予算の下で更新式と生成品質を比較する。
...
## 17. DiffusionをFlow Matchingの言葉で見る
...
### 17.3 Stochastic Interpolantsとして見る
...
## 18. 何を予測するか: noise, data, velocity
...
### 18.2 Gaussian pathからdata/noiseを解く
...
### 18.3 Flow Matching velocityとの関係
...
### 18.5 Diffusion実装からFM実装へ移るときの翻訳表
...
## 19. DiffusionとFlow Matchingは同じなのか
...
## 21. 独立couplingからOT couplingへ

> **この章で扱う概念**: 第1部で導入したcouplingを設計変数として扱い、ペア間の輸送コストを小さくするOT-CFMへ進む。

couplingの定義、独立couplingの式、`random_coupling` の役割は4.4節へ集約した。ここでは同じCFM lossを保ったまま、ペアリングを最適化すると何が変わるかを見る。
...
## 24. Linear path CFMからRectified Flowへ
...
Linear path CFMと1-Rectified Flowに共通するlossは4.5節へ集約した。この部でいう「まっすぐ」は、教師用の条件付きpathだけでなく、学習済み速度場で粒子を生成したときの実際の軌道が遠回りしないことを指す。

したがって、この部で新しく調べる対象は、学習後の軌道、Reflowによるペアの更新、少NFEでの生成品質である。

### 24.2 1-Rectified Flow

1-Rectified Flowの学習は4.3節と4.5節のLinear path CFMそのものであり、ここではlossを再導出しない。`rectified_flow_loss` は、同じ処理にRectified Flow実験上の名前を与えた入口である。

### 24.2.1 Rectified Flowの学習コード例
...
lossの式とコードは繰り返さず、ここでは学習後にEuler samplerで生成サンプルを作り、同時に `straightness_ratio` を保存する点へ注目する。これは、Rectified Flowが本当に「少ないNFEで扱いやすいまっすぐな軌道」に近づいているかを見るためである。

### 24.3 marginal preserving property
...
### 25.3.1 Rectified FlowとReflowの違い
...
# 第7部 サンプリング、ODEソルバ、NFE

## 27. solver比較の実験条件

> **この章で扱う概念**: 第1部で整理したsampler、solver、time grid、NFEを使い、solverを変えたときの離散化誤差と計算量を比較する。

samplerの定義、Euler生成、4つの用語の役割分担は4.1～5.5節へ集約した。この部では、学習済みモデルと初期乱数を同じに保ち、solverとNFEだけを変える。

### 27.1 この部で固定するものと変えるもの
...
Euler法の式、コード、1 stepあたりNFEは5.2節と5.4.1節で説明した。ここではHeunとRK4を比較するbaselineとして用いる。

### 28.2 Heun法

Heun法の直感、更新式、NFEは5.4.2節へ集約した。この部では、同じNFEのEulerより生成品質が改善するかを実験で確認する。

### 28.3 RK4

RK4の更新式、コード対応、NFEは5.4.3節へ集約した。この部では、step数ではなくNFEをそろえた比較結果を読む。
...
NFEの定義とsolverごとの回数は5.5節へ集約した。ここでは、NFEを実験予算として使う際の解釈を補う。
...
第8部のコードは、画像shapeを扱う速度場モデルの最小部品を理解するためのものである。dataset読み込み、長時間学習、重みの移動平均、複数GPU学習、画像生成評価までを含む完成済み画像生成pipelineではない。まず `model(t,x_t)` が画像と同じshapeの速度を返せるところまでを扱い、学習・生成の接続には第12章、第27章、第38章の共通パイプラインを使う。
...
### 36.2 precision/recallとcoverage
...
### 38.1 学習と生成は別のパイプラインである

学習と生成で共有する部品と、それぞれに固有の部品を図7に示す。

![Flow Matching実装の全体像](figures/implementation_parts_overview.png)

*図7　Flow Matchingにおける学習パイプラインと生成パイプライン*

**図7の読み方**: 上段は学習である。$x_0, x_1$ のペアからpathが途中点 $x_t$ と教師速度 $u_t$ を作り、モデルの予測と比較する。下段は生成である。教師速度もtargetデータも使わず、学習済みモデルをODE solverから繰り返し呼ぶ。紫色のモデルが両方に現れることが重要で、学習時に作った速度場を生成時に積分している。
...
| モデル・学習ループ・checkpoint | 第11章、第12章 |
| Diffusion schedulerと予測target | 第15章〜第18章 |
| couplingとOT-CFM | 第21章〜第23章 |
| Rectified FlowとReflow | 第24章〜第26章 |
| Euler、Heun、NFE | 第27章〜第29章 |
| U-NetとDiT | 第30章〜第33章 |
| 評価とablation | 第34章〜第37章 |

本章は、前章までの代わりではなく、実装時に机の横へ置く設計図である。「どの式を変えたのか」「どのPython部品を変えるのか」「比較時に何を固定するのか」の三点を、常に同じ行で考える。

# 第11部 学習済みFlow Matchingモデルを改造して使う

## 39. 公開済みモデル、ファインチューニング、重みの再利用
...
### 39.1 まず「Flow Matching系」の範囲を決める
...
したがって「どれもDiTだから重みを流用できる」とは限らない。第18章の予測target、第27章のsampler、第38章の部品契約を、checkpoint単位でもう一度確認する必要がある。
...
### 39.4 text encoder、VAE、Transformerを分けて考える
...
### 39.5 Flow Matchingとしてfine-tuning lossを読む
...
この順序なら、「学習が失敗した」「読み込みが失敗した」「solver設定が違う」「ライセンス上配布できない」を別々に診断できる。学習済みモデルの利用は、第38章の部品設計を実際の大規模pipelineへ拡張したものだと考えるとよい。
...
- 「Optimal Transport Conditional Flow Matching――拡散モデルに取って代わる次世代の生成技術？」. OT-CFMの直感説明を検討するために使用した日本語Web資料。
- 「Flowベース生成モデルの調査」. Flow Matchingを周辺のflow-based generative modelsと比較するために使用した日本語Web資料。
